...
business_json_map = sc.textFile(os.path.join(train_folder, 'business.json')).map(json.loads)
...
model = xgb.train({'eta': 0.3, 'booster': 'gbtree', 'max-depth': 15, 'objective': 'reg:linear', 'silent': 1}, train_data, 50)

test_rdd = sc.textFile(test_file)
test_rdd_header = test_rdd.first()
test_data = test_rdd.filter(lambda x: x != test_rdd_header)
test_data_val = test_data.map(lambda x: x.split(',')).map(lambda x: (x[0], x[1])).collect()

test_data_features = []
for test_row in test_data_val:
    test_data_features.append(get_feature_list(test_row))
test_data_features = np.asarray(test_data_features)

output = model.predict(xgb.DMatrix(test_data_features))
